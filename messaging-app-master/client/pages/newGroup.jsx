...
function NewGroup() {
  ...
  const handleGetContacts = async (signal) => {
    try {
      // thêm contact nếu trang contact được mở
      if (page.selectParticipant) {
        const { data } = await axios.get('/contacts', { signal });
        setContacts(data.payload);
      } else {
        // reset trang khi đóng
        setTimeout(() => {
          setContacts(null);
  ...
  return (
    <div
      ...
      className={`
        ${page.selectParticipant ? 'delay-75' : '-translate-x-full'}
    ...
    >
      ...
      <button
        ...
        className={`
          ${
            (modal.newGroup || selectedParticipants.length <= 0) &&
