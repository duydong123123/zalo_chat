    ...
    socket.emit(
      ...
      (cb) => {
        if (cb.success) {
          ...
          setTimeout(() => {
            // khi thành công đóng editGroup sau 0.5s
            handleCloseModal();
          }, 500);
        } else {
          setRespond(cb);
  ...
  return (
    ...
    >
      ...
      >
        {/* header */}
        <div>
          <h1 className="text-2xl font-bold">Chỉnh sửa thông tin nhóm</h1>
          {respond.message && (
            <p
              className={`mt-1 ${
          ...
          <form method="post" className="mt-4 grid" onSubmit={handleSubmit}>
            <span className="grid gap-2">
              {[
                ...
                {
                  target: 'desc',
                  placeholder: 'Thông tin nhóm (không cần thiết)',
                  required: false,
                  minLength: 0,
                  maxLength: 300,
                },
            ...
            <span className="flex gap-2 mt-4 justify-end">
              ...
              >
                <p className="font-bold text-white/90">Hoàn tất</p>
              </button>
            </span>
          </form>
        )}
